...
class _PagarPolizaScreenState extends State<PagarPolizaScreen>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    // Cuando el usuario vuelve a la app después de pagar en el navegador
    // externo, se dispara 'resumed'. Si estábamos esperando ese regreso,
    // mostramos el diálogo de confirmación automáticamente.
  ...
  Future<void> _confirmarPagoStripe() async {
    ...
    try {
      ...
      if (mounted) _irAInicioConComprobante(pago.id);
    } catch (e) {
      _mostrarError('$e');
    } finally {
      if (mounted) setState(() => _procesando = false);
  ...
  // Tras confirmar el pago, mostramos el comprobante y desde ahí el botón
  // "Ir al inicio" devuelve a la pantalla principal de la app (no a la web).
  void _irAInicioConComprobante(int pagoId) {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Se abrirá el navegador. Al terminar, vuelve a la app (botón atrás) '
              'y confirma el pago.',
