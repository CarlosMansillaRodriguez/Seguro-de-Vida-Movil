...
class CotizacionService {
  ...
  /// El CLIENTE ya no puede pasar la cotización a ACEPTADA directamente.
  /// Ese paso es exclusivo del agente (POST /cotizaciones/{id}/aceptar/).
  /// Esta función queda deprecated para el rol cliente; se mantiene solo
  /// por si el agente la reutiliza desde otra pantalla en el futuro.
  Future<Map<String, dynamic>> aceptarCotizacion(int cotizacionId) async {
    final res = await http.post(
      Uri.parse('$base/cotizaciones/$cotizacionId/aceptar/'),
      headers: auth.authHeaders,
    ...
    if (res.statusCode == 200) return jsonDecode(res.body);
    final err = jsonDecode(res.body);
    throw Exception(err['error'] ?? 'Error al aceptar cotización');
  }
  ...
  Future<Map<String, dynamic>> obtenerCotizacion(int id) async {
    final res = await http.get(
      Uri.parse('$base/cotizaciones/$id/'),
      headers: auth.authHeaders,
    );
    if (res.statusCode == 200) return jsonDecode(res.body);
    throw Exception('Error al obtener cotización (${res.statusCode})');
  }
}